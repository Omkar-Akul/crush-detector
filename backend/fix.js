...
// 1. Insert chat route before Error Handling
const chatRoute = `
// ============================================================================
...
code = code.replace(/(\/\/ ============================================================================\r?\n\/\/ ERROR HANDLING)/, chatRoute + '\n$1');


// 2. Replace app.listen with logic
const appListenPattern = /app\.listen\(PORT, \(\) => \{[\s\S]*?\}\);/;
const newListenCode = `const http = require('http');
...
code = code.replace(appListenPattern, newListenCode);

fs.writeFileSync(serverJsPath, code);
console.log('Finished correctly this time!');
